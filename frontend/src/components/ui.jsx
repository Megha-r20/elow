...
/* ─── Icons ─────────────────────────────────────────────────────── */
export const Icons = {
    Search: () => (<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
      <circle cx="8" cy="8" r="5.5"/><path d="M13.5 13.5L17 17"/>
    </svg>),
    User: () => (<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
      <circle cx="9" cy="6" r="3.5"/><path d="M2 19c0-3.866 3.134-7 7-7s7 3.134 7 7"/>
    </svg>),
    Heart: ({ filled = false }) => (<svg width="18" height="18" fill={filled ? "#E26D5C" : "none"} stroke={filled ? "#E26D5C" : "currentColor"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M9 16S2 11.5 2 6.25A4.25 4.25 0 0 1 9 3.5a4.25 4.25 0 0 1 7 2.75C16 11.5 9 16 9 16z"/>
    </svg>),
    Bag: ({ count }) => (<div style={{ position: "relative" }}>
    ...
    Close: () => (<svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
      <path d="M1 1l12 12M13 1L1 13"/>
    </svg>),
    ChevronDown: () => (<svg width="10" height="7" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1.5 1.5L5 5.5L8.5 1.5"/>
    </svg>),
    ChevronLeft: () => (<svg width="8" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M7 1L1 7l6 6"/>
    </svg>),
    ChevronRight: () => (<svg width="8" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1 1l6 6-6 6"/>
    </svg>),
    ArrowRight: () => (<svg width="16" height="12" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1 6h14M9 1l6 5-6 5"/>
    </svg>),
    Check: () => (<svg width="13" height="10" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1.5 5L5 8.5 11.5 1.5"/>
    </svg>),
    Filter: () => (<svg width="16" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
      <path d="M1 1h14M4 7h8M7 13h2"/>
    </svg>),
    Star: ({ filled = true }) => (<svg width="13" height="13" fill={filled ? "#E59866" : "#E4DCD3"} viewBox="0 0 24 24">
      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
    </svg>),
    Package: () => (<svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <path d="M20 7l-8-4-8 4M20 7v10l-8 4M20 7L12 3 4 7M4 7v10l8 4M12 3v18M4 11l8 4 8-4"/>
    </svg>),
    Truck: () => (<svg width="20" height="16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1 1h11v10H1zM12 4h4l3 3v4h-7V4z"/><circle cx="5" cy="13" r="2"/><circle cx="16" cy="13" r="2"/>
    </svg>),
    Shield: () => (<svg width="16" height="19" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
      <path d="M8 1L1 4v5c0 4.55 2.97 8.81 7 10 4.03-1.19 7-5.45 7-10V4L8 1z"/>
    </svg>),
    Tag: () => (<svg width="17" height="17" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
      <path d="M1 1h6l9 9-6 6-9-9V1z"/><circle cx="4" cy="4" r="1" fill="currentColor" stroke="none"/>
    </svg>),
    Gift: () => (<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
...
/* ─── Stars ─────────────────────────────────────────────────────── */
export function Stars({ n, size = 12, showCount, count }) {
    return (<div style={{ display: "flex", alignItems: "center", gap: showCount ? 6 : 2 }}>
      <div style={{ display: "flex", gap: 2 }}>
        {[1, 2, 3, 4, 5].map(i => (<span key={i} style={{ fontSize: size, display: "inline-block" }}>
...
/* ─── Section header ─────────────────────────────────────────────── */
export function SectionHead({ eyebrow, title, sub, right, center, }) {
    return (<div style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        {sub && (<p style={{ fontSize: 14, color: "#6E6A63", marginTop: 8, fontWeight: 400, lineHeight: 1.65, maxWidth: center ? 480 : "none" }}>
            {sub}
          </p>)}
      </div>
...
export function QtyStepper({ qty, onAdd, onSub, onChange, min = 1, max = 99 }) {
    return (<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <input type="number" min={min} max={max} value={qty} onChange={e => onChange?.(parseInt(e.target.value) || min)} style={{ width: 48, textAlign: "center", border: "1.5px solid #EAE3D9", borderRadius: 8, padding: "7px 0", fontSize: 14, fontWeight: 600, fontFamily: "inherit", outline: "none", background: "#fff", color: "#23201D" }}/>
      <button className="qty-btn" onClick={onAdd} disabled={qty >= max}>+</button>
    </div>);
}
/* ─── Empty state ───────────────────────────────────────────────── */
export function Empty({ icon, title, sub, action }) {
    return (<div style={{ textAlign: "center", padding: "72px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
...
export function LinkBtn({ to, children, style: sx }) {
    return (<Link to={to} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "#5E8C77", textDecoration: "none", transition: "gap 0.15s", ...sx }} onMouseEnter={e => (e.currentTarget.style.gap = "10px")} onMouseLeave={e => (e.currentTarget.style.gap = "6px")}>
      {children} <Icons.ArrowRight />
    </Link>);
}
/* ─── Breadcrumb ─────────────────────────────────────────────────── */
export function Breadcrumb({ items }) {
    return (<nav style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: "#9C968D", marginBottom: 24, flexWrap: "wrap" }}>
      {items.map((item, i) => (<span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {i > 0 && <span style={{ opacity: 0.5 }}>/</span>}
          {item.href ? (<Link to={item.href} style={{ color: "#9C968D", textDecoration: "none", fontWeight: 500, transition: "color 0.14s" }} onMouseEnter={e => (e.currentTarget.style.color = "#23201D")} onMouseLeave={e => (e.currentTarget.style.color = "#9C968D")}>{item.label}</Link>) : (<span style={{ color: "#23201D", fontWeight: 600 }}>{item.label}</span>)}
        </span>))}
    </nav>);
}
/* ─── Step indicator ─────────────────────────────────────────────── */
export function StepBar({ steps, current }) {
    return (<div style={{ display: "flex", alignItems: "center", gap: 0 }}>
      {steps.map((step, i) => {
            ...
            return (<div key={step} style={{ display: "flex", alignItems: "center", flex: i < steps.length - 1 ? 1 : 0 }}>
            ...
            {i < steps.length - 1 && (<div style={{ flex: 1, height: 1.5, background: done ? "#5E8C77" : "#EAE3D9", marginBottom: 22, transition: "background 0.3s" }}/>)}
          </div>);
        })}
    </div>);
}
...
export function SkeletonCard() {
    return (<div style={{ borderRadius: 18, overflow: "hidden", background: "#fff", border: "1px solid #EAE3D9" }}>
...
export function ShippingProgress({ subtotal, threshold = 999 }) {
    const rem = Math.max(0, threshold - subtotal);
    const pct = Math.min(100, (subtotal / threshold) * 100);
    return (<div style={{ background: rem === 0 ? "#EAF2EE" : "#F4EFE6", borderRadius: 12, padding: "12px 16px" }}>
      ...
      <div style={{ background: "#EAE3D9", borderRadius: 999, height: 5, overflow: "hidden" }}>
        <div className="prog-bar" style={{ height: "100%", borderRadius: 999, background: rem === 0 ? "#5E8C77" : "#5E8C77", "--pw": `${pct}%`, width: `${pct}%` }}/>
      </div>
    </div>);
}
