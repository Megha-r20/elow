...
export function AuthModal({ isOpen, onClose, initialMode = "login" }) {
  ...
  const handleGoogleAuth = async () => {
    ...
    setLoading(false);
    if (res.success) {
      addToast(`Signed in with Google as ${res.user?.name || "User"}!`, "success");
      onClose();
    } else {
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div>
                  <button
                    ...
                    className="hover:bg-[#FAF7F2] hover:border-[#23201D] active:scale-[0.99]"
                  >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 14, marginBottom: 2 }}>
                    <div style={{ flex: 1, height: 1, background: "#EAE3D9" }} />
                    <span style={{ fontSize: 10.5, fontWeight: 700, color: "#9C968D", letterSpacing: "1px" }}>OR WITH EMAIL</span>
                    <div style={{ flex: 1, height: 1, background: "#EAE3D9" }} />
                  </div>
                </div>
