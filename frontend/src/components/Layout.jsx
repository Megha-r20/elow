...
export default function Layout() {
    ...
    return (<div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
          ...
          <nav className="hide-mobile" style={{ display: "flex", gap: 28, alignItems: "center", flex: 1 }}>
            {navLinks.map(link => (link.mega ? (<div key={link.label} className="mega-wrap">
                ...
                { h: "COLLECTIONS", ls: ["The Journaling Edit", "Study Essentials", "Pastel Dreams", "Korean Desk Set"] },
                          ...
                          {col.ls.map(l => (<Link key={l} to="/shop" style={{ display: "block", fontSize: 13.5, fontWeight: 500, color: "#1C1C1A", textDecoration: "none", padding: "7px 0", borderBottom: "1px solid #F5F0E8", transition: "color 0.13s, padding-left 0.15s" }} onMouseEnter={e => { const el = e.currentTarget; el.style.color = "#5E8C77"; el.style.paddingLeft = "6px"; }} onMouseLeave={e => { const el = e.currentTarget; el.style.color = "#1C1C1A"; el.style.paddingLeft = "0"; }}>{l}</Link>))}
                        </div>))}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: "auto", flexShrink: 0, position: "relative" }}>
            ...
            {user ? (<div style={{ position: "relative" }}>
                ...
                justifyContent: "center",
                  ...
                  {isAdmin && (<span style={{ fontSize: 9.5, fontWeight: 800, background: "#5E8C77", color: "#FFFFFF", padding: "2px 7px", borderRadius: 999, letterSpacing: "0.5px" }}>
                      ADMIN
                    </span>)}
      ...
      {mobileNav && (<>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 12 }}>
              ...
              {isAdmin && (<Link to="/admin" onClick={() => setMobileNav(false)} style={{ fontSize: 16, fontWeight: 700, color: "#5E8C77", textDecoration: "none" }}>⚡ Admin Portal</Link>)}
              {user ? (<button onClick={() => { setMobileNav(false); logout(); addToast("Logged out"); }} style={{ fontSize: 16, fontWeight: 700, color: "#DC2626", background: "none", border: "none", textAlign: "left", cursor: "pointer", padding: 0 }}>Sign Out</button>) : (<button onClick={() => { setMobileNav(false); setAuthModalOpen(true); }} style={{ fontSize: 16, fontWeight: 700, color: "#5E8C77", background: "none", border: "none", textAlign: "left", cursor: "pointer", padding: 0 }}>Sign In / Register</button>)}
            </div>
          </div>
        </>)}
      ...
      {searchOpen && (<>
        ...
        } }} placeholder="Search journals, pens, washi tape, stickers…" style={{ flex: 1, fontSize: 17, border: "none", borderBottom: "2px solid #5E8C77", borderRadius: 0, padding: "8px 0", background: "transparent" }}/>
