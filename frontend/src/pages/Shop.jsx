    ...
    return (<div style={{ background: T.cream, minHeight: "100vh" }}>
      ...
      <div className="container" style={{ padding: "32px 32px" }}>
        <div style={{ display: "grid", gridTemplateColumns: (showFilters && !onlyWishlist) ? "240px 1fr" : "1fr", gap: 32, alignItems: "start" }}>
          {/* ── Sidebar filters ───────────────────────────────── */}
          {showFilters && !onlyWishlist && (<aside style={{ position: "sticky", top: 90 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 16 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input className="field field-sm" value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search products..." style={{ paddingLeft: 38, width: "100%", background: "#fff" }}/>
                </div>
                {/* Sort */}
            ...
            ].map(f => (<label key={f.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", cursor: "pointer", borderRadius: 9, transition: "background 0.14s", fontSize: 13.5, fontWeight: f.v ? 700 : 500, color: f.v ? T.txt : T.muted }} onMouseEnter={e => (e.currentTarget.style.background = T.sand)} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
              ...
              {/* Clear all filters */}
              {activeFilters.length > 0 && (<button onClick={clearAll} className="btn btn-ghost btn-sm btn-full" style={{ marginTop: 12 }}>
                  Clear all filters
                </button>)}
            </aside>)}
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, gap: 16, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                {!onlyWishlist && (<button onClick={() => setShowFilters(f => !f)} className="btn btn-ghost btn-sm" style={{ gap: 7 }}>
                    <Icons.Filter />
            ...
            : `Showing ${filtered.length} of ${liveProducts.length} products`}
              ...
              <div style={{ display: "flex", border: `1.5px solid ${T.border}`, borderRadius: 9, overflow: "hidden" }}>
                {[3, 4].map(n => (<button key={n} onClick={() => setGridView(n)} style={{ width: 34, height: 34, border: "none", cursor: "pointer", background: gridView === n ? "#1C1C1A" : "#fff", color: gridView === n ? "#fff" : T.muted, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.14s", fontSize: 11, fontWeight: 700 }}>
                    {n === 3 ? "III" : "IIII"}
                  </button>))}
              </div>
            </div>

            {filtered.length === 0 ? (<div style={{ background: "#fff", borderRadius: 20, padding: "80px 40px", textAlign: "center", border: `1px solid ${T.border}` }}>
              ...
              </div>) : (<div style={{ display: "grid", gridTemplateColumns: `repeat(${gridView}, 1fr)`, gap: 18 }}>
