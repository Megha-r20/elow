      ...
      <section className="section" style={{ background: T.cream }}>
        <div className="container">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 14 }}>
            {CATEGORIES.map(cat => (
              <button key={cat.id} onClick={() => navigate(`/shop?cat=${cat.id}`)} style={{ border: "none", background: "none", cursor: "pointer", padding: 0, textAlign: "center" }}>
                <div className="coll-card">
                  <div className="coll-img" style={{ borderRadius: 18, overflow: "hidden", height: 148, background: cat.color }}>
                    <img src={cat.image} alt={cat.label} style={{ width: "100%", height: "100%", objectFit: "cover", mixBlendMode: "multiply" }} />
                  </div>
                  <div style={{ marginTop: 10, textAlign: "center" }}>
                    <p style={{ fontSize: 11.5, fontWeight: 700, color: T.txt, letterSpacing: "0.2px" }}>{cat.label}</p>
                    <p style={{ fontSize: 11, color: T.light, marginTop: 3 }}>{cat.productCount} items</p>
                  </div>
                </div>
              </button>
            ))}
          ...
          <div className="hide-scroll" style={{ display: "flex", gap: 24, overflowX: "auto", paddingBottom: 32, paddingTop: 16, margin: "0 -32px", paddingLeft: 32, paddingRight: 32, scrollSnapType: "x mandatory" }}>
            {bestSellers.map((p, i) => (
              <div key={p.id} style={{ flex: "0 0 240px", display: "flex", flexDirection: "column", position: "relative", scrollSnapAlign: "start" }}>
                <ProductCard product={p} compact={true} badgeOverride="BESTSELLER" badgeVariantOverride="yellow" />
